...
| **Recorded Base Branch** | Base Branch a stacking tool recorded for the current branch, read from local Git data only: `branch.<current-branch>.base` configuration, then a `parentBranchName` entry in `refs/branch-metadata/<current-branch>`. It outranks the configured and detected names, and it resolves its local branch before its origin-tracking reference because a stack parent is rewritten by every restack. | Stacked branches and stacked pull requests. | A base guessed from the branch name, a base read from a remote, or Git's own configuration — Git never writes these keys. |
...
| Base branch vs upstream | A committed comparison base is not necessarily the current branch's tracking branch and must remain local and deterministic. | Use **Base Branch** for the recorded, configured, or narrowly detected comparison branch. Resolve origin-tracking refs before local refs, never use the current branch as its own fallback, and never fetch. |
| Recorded base vs name inference | Reading a base a tool recorded for a branch is not the same as guessing a base from the branch's name, which stays forbidden. | Use **Recorded Base Branch** only for values read from `branch.<name>.base` or branch-metadata objects, and invert the ref preference to local-first for those values alone. |
